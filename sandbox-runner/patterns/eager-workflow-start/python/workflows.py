...
class TransactionWorkflow:
    ...
    async def run(self, req: TransactionRequest) -> Transaction:
        # All three activities run as local activities.  When combined with
        # eager workflow start, the server dispatches the first WFT directly
        # to the co-located worker — bypassing the Matching Service queue.
