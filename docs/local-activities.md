...
<DaytonaRunner pattern="local-activities" />
...
- [Early Return + Local Activities](/early-return-local-activities) — adds an Update-with-Start early-response path on top of Local Activities for minimum first-response latency
- [Early Return](/early-return) — returns a response to the caller before the Workflow finishes, independent of Local Activities
- [Eager Workflow Start](/eager-workflow-start) — eliminates the server Matching step when starting a Workflow for additional latency reduction
- [Long Running Activity](/long-running-activity) — the right choice when Activities need heartbeating and long execution windows
