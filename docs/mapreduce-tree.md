...
4. Each Leaf Workflow calls the `processLeaf` Activity and, when complete, signals its result back to its parent Node using `signalExternalWorkflow`.
...
<DaytonaRunner pattern="mapreduce-tree" />
...
import { TASK_QUEUE, LEAF_THRESHOLD, MAX_DEPTH, RESULT_SIGNAL } from "./shared";
...
from shared import TASK_QUEUE, LEAF_THRESHOLD, MAX_DEPTH, RESULT_SIGNAL
...
// ResultSignal and ResultPayload are declared in shared.go.
...
- **Thundering herd.** The MapReduce Tree fans out exponentially. For large record sets, all leaf Activities start nearly simultaneously. Ensure your downstream system can absorb the burst, or switch to [Sliding Window](sliding-window) for rate limiting.
...
- [Fan-Out with Child Workflows](fanout-child-workflows) — simpler flat fan-out for smaller record sets
- [Sliding Window](sliding-window) — bounded concurrency with rate limiting
- [Child Workflows pattern](child-workflows) — core concepts for parent/child coordination
