...
It is not a good fit when you need concurrency limits rather than throughput limits (see [Priority Task Queues](priority-task-queues.md)), when the downstream system has no rate limit and throughput is bounded only by Workflow logic, or when all Activities require the same limit and a single shared queue suffices.
...
- **[Priority Task Queues](priority-task-queues.md)**: Route work to separate queues by urgency, with different concurrency budgets per tier.
- **[Fairness](fairness.md)**: Give each tenant an equal throughput share when multiple tenants share capacity.
